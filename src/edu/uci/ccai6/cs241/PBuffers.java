...
public class PBuffers {
	...
	public static final int __SIZE = 100;
	private int __p = 0;
	...
	private boolean __EOFInBuffer1 = false; // True: in buffer1; FALSE: in
											// buffer2
	...
	public String print() {
		...
		rsl += "[Curr char is " + getCurr() + "]\n";
		rsl += "B.1: \n";
		for (char item : __buffer1) {
			rsl += item;
		}
		rsl += "\nB.2: ";
		for (char item : __buffer2) {
			rsl += item;
		}
		rsl += "\n";
		return rsl;
	...
	public boolean hasNext() {
		...
		if (__hasEOF && (__EOFInBuffer1 == __pInBuffer1)) {
			// If __p is not out of the loadsize, then we still have chars to
			// get
	...
	public char next() {
		...
		if (__hasEOF && (__EOFInBuffer1 == __pInBuffer1)) {
			...
			if (__p >= __SIZE - 1) {
				__p = 0;
				// If we already have EOF, then don't reload buffers anymore,
				// otherwise, we can reload it
				if (!__hasEOF)
					loadBuffer(__pInBuffer1); // Reload the current buffer
				__pInBuffer1 = !__pInBuffer1; // , and move __p to another
												// buffer
			} else {
