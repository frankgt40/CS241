...
public class POPInst extends DLXInstruction {
	public POPInst(Instruction instruction) {
		...
		if (argI1 instanceof RegisterArg) {
			new DLXInstruction(new Instruction("1 SUBi " + Conf.STACK_P + " " + Conf.BLOCK_LEN + " " + Conf.STACK_P));
			__val = DLX.F1(DLX.POP, getRegNum(argI1.toString()), getRegNum(Conf.STACK_P), 0);
